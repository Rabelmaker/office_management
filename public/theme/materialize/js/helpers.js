...
const Helpers = {
  ...
  // ---
  // Redraw layouts menu (Safari bugfix)
  _redrawLayoutMenu() {
    const layoutMenu = this.getLayoutMenu()
  ...
  _setMenuHoverState(hovered) {
    this[hovered ? '_addClass' : '_removeClass']('layouts-menu-hover')
  },
  ...
  _setCollapsed(collapsed) {
    if (this.isSmallScreen()) {
      if (collapsed) {
        this._removeClass('layouts-menu-expanded')
      } else {
        setTimeout(
          () => {
            this._addClass('layouts-menu-expanded')
          },
          this._redrawLayoutMenu() ? 5 : 0
        )
      }
    } else {
      this[collapsed ? '_addClass' : '_removeClass']('layouts-menu-collapsed')
    }
  },

  // ---
  // Add layouts sivenav toggle animationEnd event
  ...
  // ---
  // Remove layouts sivenav toggle animationEnd event
  ...
  _bindMenuMouseEvents() {
    ...
    if (!this._menuMouseEnter) {
      this._menuMouseEnter = () => {
        if (
          this.isSmallScreen() ||
          !this._hasClass('layouts-menu-collapsed') ||
          this.isOffcanvas() ||
          this._hasClass('layouts-transitioning')
        ) {
    ...
    if (!this._windowTouchStart) {
      this._windowTouchStart = e => {
        if (!e || !e.target || !this._findParent(e.target, '.layouts-menu')) {
          this._setMenuHoverState(false)
        }
      }
  ...
  setCollapsed(collapsed = requiredParam('collapsed'), animate = true) {
    ...
    if (animate && this._supportsTransitionEnd()) {
      this._addClass('layouts-transitioning')
      ...
      this._bindLayoutAnimationEndEvent(
        ...
        () => {
          this._removeClass('layouts-transitioning')
      ...
      this._addClass('layouts-no-transition')
      if (collapsed) this._setMenuHoverState(false)
      ...
      setTimeout(() => {
        this._removeClass('layouts-no-transition')
        this._triggerWindowEvent('resize')
        this._triggerEvent('toggle')
        this._setMenuHoverState(false)
      }, 1)
  ...
  // ---
  // Set layouts positioning
  setPosition(fixed = requiredParam('fixed'), offcanvas = requiredParam('offcanvas')) {
    this._removeClass('layouts-menu-offcanvas layouts-menu-fixed layouts-menu-fixed-offcanvas')

    if (!fixed && offcanvas) {
      this._addClass('layouts-menu-offcanvas')
    } else if (fixed && !offcanvas) {
      this._addClass('layouts-menu-fixed')
      this._redrawLayoutMenu()
    } else if (fixed && offcanvas) {
      this._addClass('layouts-menu-fixed-offcanvas')
      this._redrawLayoutMenu()
    }
  ...
  getLayoutMenu() {
    return document.querySelector('.layouts-menu')
  },
  ...
  getLayoutNavbar() {
    return document.querySelector('.layouts-navbar')
  },
  ...
  getLayoutContainer() {
    return document.querySelector('.layouts-page')
  },
  ...
  setNavbarFixed(fixed = requiredParam('fixed')) {
    this[fixed ? '_addClass' : '_removeClass']('layouts-navbar-fixed')
    this.update()
  },

  setNavbar(type) {
    if (type === 'sticky') {
      this._addClass('layouts-navbar-fixed')
      this._removeClass('layouts-navbar-hidden')
    } else if (type === 'hidden') {
      this._addClass('layouts-navbar-hidden')
      this._removeClass('layouts-navbar-fixed')
    } else {
      this._removeClass('layouts-navbar-hidden')
      this._removeClass('layouts-navbar-fixed')
    }
  ...
  setFooterFixed(fixed = requiredParam('fixed')) {
    this[fixed ? '_addClass' : '_removeClass']('layouts-footer-fixed')
    this.update()
  },
  ...
  setContentLayout(contentLayout = requiredParam('contentLayout')) {
    setTimeout(() => {
      ...
      const navbarArea = document.querySelector('.layouts-navbar') // For navbar area for vertical menu
      const navbarAreaHorizontal = document.querySelector('.layouts-navbar > div') // For navbar area for horizontal
      const navbarSearchInputWrapper = document.querySelector('.layouts-navbar .search-input-wrapper') // For navbar search input wrapper
      const navbarSearchInput = document.querySelector('.layouts-navbar .search-input-wrapper .search-input') // For navbar search input
      ...
      // Condition to check if layouts is horizontal menu
  ...
  isLayoutNavbarFull() {
    return !!document.querySelector('.layouts-wrapper.layouts-navbar-full')
  },

  isCollapsed() {
    if (this.isSmallScreen()) {
      return !this._hasClass('layouts-menu-expanded')
    }
    return this._hasClass('layouts-menu-collapsed')
  },

  isFixed() {
    return this._hasClass('layouts-menu-fixed layouts-menu-fixed-offcanvas')
  },

  isOffcanvas() {
    return this._hasClass('layouts-menu-offcanvas layouts-menu-fixed-offcanvas')
  },

  isNavbarFixed() {
    return (
      this._hasClass('layouts-navbar-fixed') || (!this.isSmallScreen() && this.isFixed() && this.isLayoutNavbarFull())
    )
  },

  isFooterFixed() {
    return this._hasClass('layouts-footer-fixed')
  },
...
if (typeof window !== 'undefined') {
  ...
  if (Helpers.isMobileDevice() && window.chrome) {
    document.documentElement.classList.add('layouts-menu-100vh')
  }

  // Update layouts after page load
