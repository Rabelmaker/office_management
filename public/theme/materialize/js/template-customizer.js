...
if (cl.contains('layouts-navbar-fixed')) layoutNavbarVar = 'sticky'
else if (cl.contains('layouts-navbar-hidden')) layoutNavbarVar = 'hidden'
...
const DEFAULT_MENU_COLLAPSED = !!cl.contains('layouts-menu-collapsed')
const DEFAULT_SHOW_DROPDOWN_ON_HOVER = true
const DEFAULT_NAVBAR_FIXED = layoutNavbarVar
const DEFAULT_CONTENT_LAYOUT = cl.contains('layouts-wide') ? 'wide' : 'compact'
const DEFAULT_FOOTER_FIXED = !!cl.contains('layouts-footer-fixed')

let headerType
if (cl.contains('layouts-menu-offcanvas')) {
  headerType = 'static-offcanvas'
} else if (cl.contains('layouts-menu-fixed')) {
  headerType = 'fixed'
} else if (cl.contains('layouts-menu-fixed-offcanvas')) {
...
class TemplateCustomizer {
  ...
  setLayoutType(pos, updateStorage = true) {
    ...
    if (this.settings.headerType === 'fixed' || this.settings.headerType === 'fixed-offcanvas') {
      // Set perfectscrollbar wheelPropagation false for fixed layouts
      ...
      // Destroy perfectscrollbar for static layouts
      window.Helpers.menuPsScroll.destroy()
    }
  }
  ...
  setDropdownOnHover(open, updateStorage = true) {
    ...
    if (window.Helpers.mainMenu) {
      ...
      window.Helpers.mainMenu = new Menu(document.getElementById('layouts-menu'), {
  ...
  update() {
    ...
    const hasNavbar = !!document.querySelector('.layouts-navbar')
    const hasMenu = !!document.querySelector('.layouts-menu')
    const hasHorizontalMenu = !!document.querySelector('.layouts-menu-horizontal.menu, .layouts-menu-horizontal .menu')
    const isLayout1 = !!document.querySelector('.layouts-wrapper.layouts-navbar-full')
    ...
    if (this._controls.layoutNavbarOptions) {
      ...
      //  Horizontal menu fixed layouts - disabled fixed navbar switch
    ...
    if (this._controls.headerType) {
      // ? Uncomment If using offcanvas layouts
      /*
      if (!hasMenu) {
  ...
  _setup(_container = document) {
    ...
    // Layout wrapper
    const layoutW = this.container.querySelector('.template-customizer-layouts')
    ...
    if (!this._hasControls('rtl headerType contentLayout layoutCollapsed layoutNavbarOptions', true)) {
      ...
      if (!this._hasControls('layoutCollapsed')) {
        ...
        setTimeout(() => {
          if (document.querySelector('.layouts-menu-horizontal')) {
            layoutCollapsedW.parentNode.removeChild(layoutCollapsedW)
          }
        }, 100)
    ...
    setTimeout(() => {
      const layoutCustom = this.container.querySelector('.template-customizer-layouts')
  ...
  // Set settings in LocalStorage with layouts & key
  _getSetting(key) {
    let result = null
    const layoutName = this._getLayoutName()
    try {
  ...
  // Get layouts name to set unique
  _getLayoutName() {
